...
def XYZbinning_COO(
    ...
    """
    Same accumulation as XYZbinning_numba, but returns only
    non-zero voxels in COO format instead of a dense volume.

    Returns
    -------
    z_coords  : np.ndarray uint16  shape (N,)
    y_coords  : np.ndarray uint16  shape (N,)
    x_coords  : np.ndarray uint16  shape (N,)
    signal_vals : np.ndarray float32 shape (N,)
    """
    if shifts is None:
    ...
    print(f"[INFO] COO: {len(vals)} non-zero voxels / {Z*H_out*W_out} total "
          f"({100*len(vals)/(Z*H_out*W_out):.1f}% fill)")

    return z_c.astype(np.uint16), y_c.astype(np.uint16), x_c.astype(np.uint16), vals
