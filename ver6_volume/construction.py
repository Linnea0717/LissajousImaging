...
def processDataset(dataset_name: str, z_slices: int, out_w: int, out_h: int):
    ...
    save_dir = PROJECT_ROOT / "output" / "ver6" / dataset_name
    ...
    for i, volume_x, volume_z in volume_gen:
        ...
        z_c, y_c, x_c, vals = XYZbinning_COO(
        ...
        coo_list.append((z_c, y_c, x_c, vals))
    ...
    for i, (z_c, y_c, x_c, vals) in enumerate(coo_list):
        np.savez_compressed(
            save_base / f'vol_{i:04d}.npz',
            x=x_c, y=y_c, z=z_c,
            signal=vals,
            shape=np.array([z_slices, out_h, out_w], dtype=np.uint16)
        )
